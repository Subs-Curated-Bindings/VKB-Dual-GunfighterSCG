<#
.SYNOPSIS
    Bindings Toolkit for [Enhanced] Dual VKB Gunfighter Binds. Menu-driven utility
...
#  Refactored from the original Fix MFD Binds [ENH][GF].ps1.
# =====================================================================
...
function Invoke-FixMfd-Channel {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        Write-Host "  Load the Gunfighter layout in-game first (Customization > Control Profiles)," -ForegroundColor Yellow
...
function Invoke-ClearAllBinds-Channel {
    ...
    Write-Host "    2. Customization > Control Profiles > Use this profile -- pick the Gunfighter layout." -ForegroundColor Cyan
...
function Test-LayoutXmlForChannel {
    # Returns @{ Present; Path; SizeKB; MTime } -- the Gunfighter layout file
    ...
    $found = @(Get-ChildItem -LiteralPath $mappings -Filter 'layout_ENH_GF_*_exported.xml' -ErrorAction SilentlyContinue)
    if ($found.Count -eq 0) {
        return @{ Present = $false; Reason = 'no layout_ENH_GF_*_exported.xml in mappings dir' }
    }
...
function Show-StackHealth {
    ...
    elseif ($vj.DeviceCount -lt 2) {
        Write-Host ("  vJoy: {0} loaded, {1} device(s) configured -- Gunfighter profile expects 2 (use vJoyConf to add a second)" -f $vVer, $vj.DeviceCount) -ForegroundColor Yellow
    }
    else {
...
function Invoke-ShowDiagnostic-Channel {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        elseif ($slots.Count -eq 1) {
            Write-Host ("  vJoy slots in use: {0} only -- Gunfighter profile expects both js1 and js2" -f $slots[0]) -ForegroundColor Yellow
        }
        else {
