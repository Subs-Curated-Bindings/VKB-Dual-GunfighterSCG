<#
.SYNOPSIS
    Re-applies the [Enhanced] Dual VKB Gunfighter Binds MFD binds to Star
...
.DESCRIPTION
    ...
    WORKFLOW:
      1. Load the [ENH][GF] layout in-game once (Customization > Control
...
.EXAMPLE
    .\Fix MFD Binds [ENH][GF][4.8.0][LIVE].ps1
    Auto-detect channels, prompt to pick.

.EXAMPLE
    .\Fix MFD Binds [ENH][GF][4.8.0][LIVE].ps1 -Channel PTU

.EXAMPLE
    .\Fix MFD Binds [ENH][GF][4.8.0][LIVE].ps1 -InstallRoot 'D:\Games\StarCitizen' -Channel PTU
#>
[CmdletBinding()]
param(
...
# ===================================================================
#  STICK-SPECIFIC BIND DATA -- VKB Gunfighter MFD layout
#  Do not edit unless you are customizing your own Gunfighter MFD bindings.
# ===================================================================
$Binds = @(
...
if (-not (Test-Path -LiteralPath $InstallRoot)) {
    ...
    if (-not (Test-Path -LiteralPath $entered)) {
        ...
        Write-Host "  .\Fix MFD Binds [ENH][GF][4.8.0][LIVE].ps1 -InstallRoot 'X:\path\to\StarCitizen'" -ForegroundColor Yellow
        exit 1
    }
    $InstallRoot = $entered
    Write-Host ""
